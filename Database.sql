...
--get user details from token (doesn't work)
CREATE PROCEDURE GetDetailsUsingToken
@Token VARCHAR(25),
@Email BIT OUTPUT,
...
BEGIN
	BEGIN TRANSACTION
		...
		IF EXISTS (SELECT Customer_ID FROM Sessions WHERE Token = @Token AND TimeStamp <= ExpiryTime)
		--CHECK IF IN DATE
			BEGIN
				DECLARE @CustomerID AS INT = (SELECT Customer_ID FROM Sessions WHERE Token = @Token);
				--get all details
				IF EXISTS (SELECT * FROM Customer WHERE Customer_ID = @CustomerID AND Amin = 0)
					BEGIN
						SELECT @Email = SELECT Email_Address FROM Customer WHERE Customer_ID = @CustomerID;
						
						SELECT @First_Name = SELECT First_Name FROM Customer WHERE Customer_ID = @CustomerID;
						
						SELECT @Last_Name = SELECT Last_Name FROM Customer WHERE Customer_ID = @CustomerID;
						
						SELECT @Age = SELECT Age FROM Customer WHERE Customer_ID = @CustomerID;
						
						SELECT @Address = SELECT Address FROM Customer WHERE Customer_ID = @CustomerID;
						
						SELECT @PhoneNumber = SELECT Phone_Number FROM Customer WHERE Customer_ID = @CustomerID;
